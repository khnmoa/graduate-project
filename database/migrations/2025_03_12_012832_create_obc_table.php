...
return new class extends Migration
{
    ...
    {
        Schema::create('obcs', function (Blueprint $table) {
            $table->id(); // Primary key (Auto-incrementing ID)

              // مفاتيح أجنبية تربط الجدول بجدول power, telemetry, communications
              $table->foreignId('power_id')->constrained('powers')->onDelete('cascade');
              $table->foreignId('telemetry_id')->constrained('telemetries')->onDelete('cascade');
              $table->foreignId('communications_id')->constrained('communications')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('obcs'); // Deletes the 'obc' table if it exists
    }
};
